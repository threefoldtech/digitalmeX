...
class GedisServer(JSBaseConfig, StreamServer):
    _SCHEMATEXT = """
    ...
    def _init(self):
        ...
        self.code_generated_dir = j.sal.fs.joinPaths(
            j.dirs.VARDIR, "codegen", "gedis", self.name, "server")
        ...
        for cat in ["server", "client"]:
            code_generated_dir = j.sal.fs.joinPaths(
                j.dirs.VARDIR, "codegen", "gedis", self.name, cat)
            j.sal.fs.remove(code_generated_dir)
            j.sal.fs.createDir(code_generated_dir)
            j.sal.fs.touch(j.sal.fs.joinPaths(
                code_generated_dir, '__init__.py'))

        # now add the one for the server
        if self.code_generated_dir not in sys.path:
            sys.path.append(self.code_generated_dir)

        self.actors_add(namespace="system", path="%s/systemactors" %
                        j.servers.gedis._dirpath)  # add the system actors
    ...
    def actor_add(self, path, namespace="default"):
        ...
        self.cmds_meta[key] = GedisCmds(
            server=self, path=path, name=name, namespace=namespace)
    ...
    def client_get(self, namespace="default"):
        ...
        data["host"] = self.host
        data["port"] = self.port
        data["adminsecret_"] = self.password
        data["ssl"] = self.ssl
        data["namespace"] = namespace

        return j.clients.gedis.get(instance=self.name, data=data, reset=False, configureonly=False)
    ...
    def client_configure(self, namespace="default"):
        ...
        data["host"] = self.host
        data["port"] = self.port
        data["adminsecret_"] = self.password_
        data["ssl"] = self.ssl
        data["namespace"] = namespace

        return j.clients.gedis.get(instance=self.name, data=data, reset=False, configureonly=True)
    ...
    def job_schedule(self, method,  timeout=60, wait=False, depends_on=None, **kwargs):
        ...
        """
        job = self.workers_queue.enqueue_call(
            func=method, kwargs=kwargs, timeout=timeout, depends_on=depends_on)
    ...
    def sslkeys_generate(self):
        if self.ssl:
            ...
            if res:
                ...
                self._logger.info(
                    'using existing key and cerificate for gedis @ %s' % path)
    ...
    #     self.websocket_server = pywsgi.WSGIServer(('0.0.0.0', 9999), self.websocketapp, handler_class=WebSocketHandler)
        ...
        # self.jsapi_server.instance = self.name
