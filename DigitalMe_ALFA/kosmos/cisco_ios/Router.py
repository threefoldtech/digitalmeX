...
class Router(j.application.JSBaseClass):
    def __init__(self, hostname, software=IOS, logfile=None, **kwargs):
        ...
        if self.software == IOS:
            ...
            self.PROMPT_PRIVILEGED_EXEC_MODE = prompt_syntax_xr + hostname + "#"
            self.PROMPT_GLOBAL_CONFIG_MODE = prompt_syntax_xr + hostname + "\(config\)#"
            self.PROMPT_CONFIG_SUB_MODE = prompt_syntax_xr + hostname + "\(config-.*\)#"

        if logfile is None:
            direc = os.getcwd()
            logfilename = self.hostname + "-" + time.strftime("%m-%d-%Y_%H-%M-%S") + ".log"
    ...
    def login(self, cmd, username=None, password=None, final_prompt=None):
        ...
        expect_list = [self._LOGIN_YES_PROMPTS, self._LOGIN_USERNAME_PROMPTS, self._LOGIN_PASSWORD_PROMPTS]

        i = self._pexpect_session.expect(expect_list)
        matched = expect_list[i]

        if i == expect_list.index(self._LOGIN_YES_PROMPTS):
            matched = self.exec_cmd("yes", expects=expect_list, return_matched=True)
            try:
                i = expect_list.index(matched)
            except ValueError:
                msg = "After giving yes, {0} not in list {1}" "".format(matched, repr(expect_list))
                self._log(msg, level="WARNING")

        if i == expect_list.index(self._LOGIN_USERNAME_PROMPTS):
            if username is None:
                self._exit("Username not provided")
            matched = self.exec_cmd(username, expects=expect_list, return_matched=True)
            try:
                i = expect_list.index(matched)
            except ValueError:
                msg = "After giving username, {0} not in list {1}" "".format(matched, repr(expect_list))
                self._log(msg, level="WARNING")
        ...
        if i == expect_list.index(self._LOGIN_PASSWORD_PROMPTS):
            ...
            try:
                ...
                msg = "After giving password, {0} not in list {1}" "".format(matched, repr(expect_list))
                self._log(msg, level="WARNING")

            if i == expect_list.index(self._LOGIN_USERNAME_PROMPTS):
                msg = (
                    'Prompt: "{0}" returned again... Indicates '
    ...
    def exec_cmd(self, cmd, **kwargs):
        ...
        self._log("Output for previous cmd: '{0}'" "".format(repr(output)))

        matched = expects[i]
        if i == len(expects) - 1:  # If EOF
            msg = """Expected Prompt not found
                      Match list: {0}
                      Output After Prompt: '{1}'""".format(
                expects, repr(self._pexpect_session.after)
            )
            self._exit(msg)

        elif i != len(expects) - 2:  # If not Current Prompt
            msg = (
                "Warning: Last command matched: '{0}'."
            ...
            if (i == len(expects) - 3) and auto_complete is True:
                ...
                while more_seen:
                    ...
                    new_expects = [MORE_OUTPUT_PROMPT, self._prompt_current, pexpect.EOF]
                    ...
                    if j == 2:
                        msg = """Expected Prompt not found
                                  Match list: {0}
                                  Output After Prompt: '{1}'""" """""".format(
                            new_expects, repr(self._pexpect_session.after)
                        )
                        self._exit(msg)
                    elif j == 1:
                        self._log("{0} stopped showing. Matched {1} finally" "".format(new_expects[0], new_expects[1]))
                        break
                    elif i == 0:
                        self._log("{0} seen {1} times" "".format(new_expects[0], cnt))
                        cnt += 1
    ...
    def _exit(self, msg=""):
        if self._pexpect_session.isalive() is True:
            try:
                ...
                expect_list = ["Connection closed", "closed", "connection"]
                self._pexpect_session.expect(expect_list)
            except Exception as e:
                msg1 = "Exception caught during closing " "connection: {0}".format(e)
