...
def chat(bot):
    ...
    while True:
        ...
        repo_url = bot.string_ask("Enter website repo url: (example: https://github.com/Incubaid/www_incubaid)", validate={"required":True,"github_url_https":True})
        repo_branch = bot.string_ask("Branch to deploy from", default="master")
        jwt = bot.string_ask("Enter your JWT token  <a target='_blank' href='https://github.com/threefoldtech/jumpscaleX/blob/development/docs/howto/get_jwt_with_itsoyouonline_client.md'>(?)</a>",validate={"required":True, "jwt":True})
        farm_zerotier_id = bot.string_ask("Enter Farm Zerotier ID", validate={"required":True})
        server_type = bot.single_choice("What will the website run on ", ['caddy', 'ngnix and lapis'], validate={"required":True})
        ...
        def container_create():
            ...
            if server_type == 'caddy':
                ...
                app_file_location = bot.string_ask("Add path to the application file (Extension .moon) inside the repo ", validate={"required":True})
